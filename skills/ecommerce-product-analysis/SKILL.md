...
把 `$PRODUCT_PHOTOS`（来自 `$DIR/.anbanwriter/products/index.json`，见主流程「产品图发现」）写入 `$DIR/product-photos.md`，逐张标注序号、来源路径、可见性（是否可访问/是否需压缩）。任一不可访问剔除并记录，**至少保留 1 张**；全不可访问则报错停止。

### 步骤 2：逐张抽取产品属性

对**每张**产品图调 `analyze_image`（逐张单张识别，一次一张），使用下方**电商属性抽取 prompt**。一张图可能只展示部分属性（如包装图只见包装文字，场景图见使用语境），多张互补。

**`subject` 部位标签是关键产物**——它决定后续生成时「这张图该被哪些电商图当参考」。务必为每张图给出明确的 subject（茶汤/干茶/叶底/包装正面/细节特写/使用场景…），看不清的标注置信 low。
...
{
  "subject": "这张图展示的是产品的哪个部位/视角/状态？例：包装正面/包装背面/产品本体/原料/成品/剖面/细节特写/使用场景/模特手持；茶叶类可为：茶汤/干茶/叶底/包装。该字段决定本图后续被哪些电商图选作参考。",
...
|------|------------------|-------------------|----------------------------------------------|
| 1    | 包装正面         | `$DIR/.anbanwriter/products/product_01.png` | 品牌文字「XX」/ 主色 #XXX / 哑光金盒 |
