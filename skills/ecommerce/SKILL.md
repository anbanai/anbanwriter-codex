...
- `get_project_profile(project_id="$PROJECT_ID", scope="ecommerce", task_id="$TASK_ID")` → 品牌定位/受众/参考图风格/**已解析的 `image_model{provider,model,key}`**（建任务时由用户 `image_model_key` 选定）。`task_id` 必传以继承模板派生风格与任务级模型。
...
- 锚点优先（主图①先确立基准）→ 按模块逐张生成（产品档案前缀块 + 按 `image_model.provider` 适配的参考图策略：OpenAI/Gemini 多参考 / Seedream 单锚点 + 产品档案文本块 + `verify_with_vision` 自检 + 3 轮收敛）
